...
export default function WriteReviewPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchReviewData = async () => {
      if (isEditing && apiService) {
        try {
          const review = await apiService.getCourtReview(params.id, reviewId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newPhotoIds = await Promise.all(images.map(file => uploadPhoto(file)));
    ...
    <div className="max-w-[800px] mx-auto px-4 py-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Share your dining experience at this court</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground mt-2">You can upload up to 8 photos</p>
            </div>
            ...
            <div className="flex gap-4">
              <Button
                type="button"
                variant="outline"
                onClick={() => router.push(`/courts/${params.id}`)}
              >
                Cancel
              </Button>
              <Button type="submit" disabled={!rating || !content || isSubmitting}>
                {isSubmitting ? 'Uploading...' : isEditing ? 'Update Review' : 'Submit Review'}
              </Button>
            </div>
          </form>
        </CardContent>
